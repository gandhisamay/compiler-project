...
void resolve(TreeNode *node) {
  ...
  if (node->symbol->is_terminal) {
    ...
    if (node->head == NULL) {
      ...
      // input_plist
      AST_Node *tmp =
          copy_AST_Node(node->head->sibling->sibling->sibling->sibling->sibling
                            ->sibling->sibling->list->head);
      while (tmp != NULL) {
      ...
      //     node->head->sibling->sibling->sibling->head->sibling->sibling; int
      //     start, end;
      //     // Start Index_arr
      ...
      //         // TODO: FIX Offset
      //         SYMBOL_TABLE_ELEMENT *el =
      //         create_symbol_table_element(ID->token.id, true, ID->token.name,
      //         start, end, 0, ID->token.line); insert_symbol_table(el,
      //         scope->table); ID = ID->next;
      //     }
      // }
      ...
      if (node->head->sibling == NULL) {
        ...
        insert_AST_head(node->list, node->head->sibling->sibling->sibling->list
      ...
      resolve(
          node->head->sibling->sibling->sibling->sibling->sibling); // CaseStmts
      resolve(node->head->sibling->sibling->sibling->sibling->sibling
                  ->sibling); // Default
      append_AST_lists_tail(node->list,
                            node->head->sibling->sibling->sibling->sibling
                                ->sibling->list); // appending CaseStmts
      append_AST_lists_tail(node->list, node->head->sibling->sibling->sibling
      ...
      insert_AST_head(node->list, case_node);
      ...
      if (node->head->sibling == NULL) {
        ...
        insert_AST_head(node->list, stmts_end);
      }
      ...
      // Add to symbol table
      Scope *scope = find_scope(GLOBAL_SCOPE,
                                node->head->token.line); // line no of 'dECLARE'
      ...
      if (node->head->sibling->sibling->sibling->head->sibling ==
        ...
        if (range_array->head->sibling->sibling->head->sibling ==
          ...
          if (range_array->head->sibling->sibling->head->head->token.name ==
              pLUS) {
            if (range_array->head->sibling->sibling->head->sibling->head->token
                    .name == nUM) {
            ...
            if (range_array->head->sibling->sibling->head->sibling->head->token
                    .name == nUM) {
              end = 0 - range_array->head->sibling->sibling->head->sibling->head
                            ->token.num;
            } else { // iD type
              nUM_or_iD = 0;
            }
        ...
        while (ID != NULL) {
          // TODO: FIX Offset
          SYMBOL_TABLE_ELEMENT *el =
              create_symbol_table_element(ID->token.id, true, type, start, end,
                                          CURR_OFFSET, ID->token.line);
          insert_symbol_table(el, scope->table);
          ID = ID->next;
          if (nUM_or_iD) {
            if (type == iNTEGER) {
    ...
    // }
      ...
      if (node->head->symbol->terminal == fOR) {
        ...
        AST_Node *FORLOOP_END = create_AST_Node("FORLOOP_END", NULL);
        AST_Node *STATEMENTS = create_AST_Node("STATEMENTS", NULL);
        AST_Node *STATEMENTS_END = create_AST_Node("STATEMENTS_END", NULL);
        ...
        append_AST_lists_tail(node->list,
                              node->head->sibling->sibling->sibling->sibling
                                  ->sibling->sibling->sibling->list);
        insert_AST_tail(node->list, STATEMENTS_END);
        insert_AST_tail(node->list, FORLOOP_END);
      } else {
      ...
      if (node->head->sibling == NULL) {
        ...
        resolve(node->head->sibling); // sign
      ...
      // AST_Node *PARAMETERS_END = create_AST_Node("PARAMETERS_END", NULL);
